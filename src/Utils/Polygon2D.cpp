...
bool Polygon2D::IsPointInside(const double p[])
{
	...
	if (IsPointInsideBox2D(p, &bbox[0], &bbox[2]) == false)
		return false;
...
void Polygon2D::SetupFromParams(Params &p)
{
	...
	{
		Logger::m_out << "warning Polygon2D::SetupFromParams ... odd size " << m_vertices.size() << "... removing last value" << std::endl;
		m_vertices.pop_back();
	}
	OnShapeChange();
	MakeCCW();
...
Status Polygon2D::Read(std::istream &in)
{
	...
	for (int i = 0; i < 2 * n; ++i)
	{
		if (!(in >> m_vertices[i]))
		{
			...
			Logger::m_out << "error Polygon2D::Read ... expecting value " << i << " for polygon with " << n << " vertices " << std::endl;
			return STATUS_ERROR;
		}
	}
