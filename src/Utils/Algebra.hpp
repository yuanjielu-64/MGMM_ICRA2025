...
{
	const double c = VecDotProduct(n, v1, v2) / (VecNorm(n, v1) * VecNorm(n, v2));
	return fabs(c - 1) <= Constants::EPSILON ? Constants::PARALLEL : (fabs(c + 1) <= Constants::EPSILON ? Constants::ANTI_PARALLEL : Constants::NEITHER_PARALLEL_NOR_ANTI_PARALLEL);
}
...
{
	const double d = VecDotProduct(n, v1, v2);
	return fabs(1 - d) <= Constants::EPSILON ? Constants::PARALLEL : (fabs(1 + d) < Constants::EPSILON ? Constants::ANTI_PARALLEL : Constants::NEITHER_PARALLEL_NOR_ANTI_PARALLEL);
}
