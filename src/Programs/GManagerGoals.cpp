    ...
    else if (item == MENU_DRAW_GOALS_COLOR_PATH)
    {
        if (GetSetup()->GetProblem()->GetGoals()->size() > 0)
        {
            ...
            auto curr = std::find_if(GetManager()->GetComponents()->begin(),
                                     ...
                                     [](GManagerComponent *comp) { return dynamic_cast<GManagerDecomposition *>(comp) != NULL; });
            if (curr != GetManager()->GetComponents()->end())
                gid = dynamic_cast<GManagerDecomposition*>(*curr)->GetSelectedGoal();

            GetSetup()->GetDecomposition()->SetRegionColorsBasedOnPathCost(gid, Region::STATUS_FREE | Region::STATUS_GOAL);
        }
        return true;
    }
...
int GManagerGoals::PrepareMenu(void)
{
    ...
    GetManager()->AddMenuEntry((*items)[MENU_DRAW_GOALS_COLOR_CLEARANCE]->GetExtendedName(), MENU_DRAW_GOALS_COLOR_CLEARANCE);
