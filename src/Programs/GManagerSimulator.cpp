...
void GManagerSimulator::HandleEventOnDisplay(void)
{
  ...
  if (m_target[0] != INFINITY && HasAllFlags((*items)[MENU_STEER]->GetFlags(), GMenuItem::FLAG_ON))
  {
    ...
    GDrawSphere3D(m_target, sim ? sim->GetDistanceReachedSteerPosition() : Constants::SIMULATOR_DISTANCE_REACHED_STEER_POSITION);
  }

  
  if (m_cfgs[0] != NULL && m_cfgs[1] != NULL  && HasAllFlags((*items)[MENU_PATH]->GetFlags(), GMenuItem::FLAG_ON))
  {
...
bool GManagerSimulator::HandleEventOnMouseLeftBtnDown(const int x, const int y)
{
  ...
  if (GetSetup()->GetSimulator() && HasAllFlags((*items)[MENU_STEER]->GetFlags(), GMenuItem::FLAG_ON))
  {
    ...
    Logger::m_out << "steering target: " << m_target[0] << " " << m_target[1] << " " << m_target[2] << std::endl;
...
void GManagerSimulator::HandleEventOnTimer(void)
{
  ...
  if (GetSetup()->GetSimulator() && HasAllFlags((*items)[MENU_STEER]->GetFlags(), GMenuItem::FLAG_ON) && m_target[0] != INFINITY)
  {
