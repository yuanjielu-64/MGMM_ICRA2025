...
void GManagerDecomposition::HandleEventOnDisplay(void)
{
        ...
        dynamic_cast<DecompositionGeometric *>(decomp))
    {
        ...
        dynamic_cast<DecompositionGeometric *>(decomp)->DrawRegionsShape(Region::STATUS_FREE | Region::STATUS_REGULAR);
    }
...
bool GManagerDecomposition::HandleEventOnMouseLeftBtnDown(const int x, const int y)
{
    ...
    if (GetSetup()->GetDecomposition() && HasAllFlags((*items)[MENU_SELECT_REGION]->GetFlags(), GMenuItem::FLAG_ON))
    {
	Logger::m_out << "................YES " << std::endl;
        ...
        if (m_idSelectedRegion >= 0)
            Logger::m_out << "selected region " << m_idSelectedRegion << " with status " << dynamic_cast<Region *>(GetSetup()->GetDecomposition()->GetGraph()->GetVertex(m_idSelectedRegion))->GetFlags() << std::endl;
	else
...
bool GManagerDecomposition::HandleEventOnMenu(const int item)
{
    ...
    {
        GetSetup()->GetDecomposition()->SetRegionColorsBasedOnClearance(Region::STATUS_FREE | Region::STATUS_REGULAR);
        return true;
    }
    else if (item == MENU_DRAW_REGIONS_COLOR_PATH)
    {
        if (GetSetup()->GetProblem()->GetGoals()->size() > 0)
            GetSetup()->GetDecomposition()->SetRegionColorsBasedOnPathCost(m_indexSelectedGoal >= 0 ? m_indexSelectedGoal : 0,
                                                                           Region::STATUS_FREE | Region::STATUS_REGULAR);
        return true;
    }
    else if (item == MENU_DRAW_REGIONS_COLOR_SAME)
    {
        GetSetup()->GetDecomposition()->SetRegionColors(GetSetup()->GetColor(Setup::COLOR_DECOMPOSITION_REGIONS_INSIDE),
                                                        Region::STATUS_FREE | Region::STATUS_REGULAR);
        return true;
    }
    else
...
int GManagerDecomposition::PrepareMenu(void)
{
    ...
    GetManager()->AddMenuEntry((*items)[MENU_DRAW_REGIONS_COLOR_SAME]->GetExtendedName(), MENU_DRAW_REGIONS_COLOR_SAME);
    GetManager()->AddMenuEntry((*items)[MENU_DRAW_REGIONS_COLOR_CLEARANCE]->GetExtendedName(), MENU_DRAW_REGIONS_COLOR_CLEARANCE);
    GetManager()->AddMenuEntry((*items)[MENU_DRAW_REGIONS_COLOR_PATH]->GetExtendedName(), MENU_DRAW_REGIONS_COLOR_PATH);
