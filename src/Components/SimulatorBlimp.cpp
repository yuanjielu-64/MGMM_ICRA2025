...
void SimulatorBlimp::SetupFromParams(Params &params)
{
  ...
  auto name = params.GetValue(Constants::KW_VehicleMeshFile, Constants::BLIMP_MESH_FILE);
  if(name)
  {
...
void SimulatorBlimp::CompleteSetup(void)
{
  ...
  m_meshCollision.AddBox(-GetBodyLength(), -0.5 * GetBodyWidth(), -0.5 * GetBodyHeight(), 0.0, 0.5 * GetBodyWidth(), GetBodyHeight());
  
  if(m_meshDraw.GetNrVertices() == 0)
      m_meshDraw.AddTriMesh(m_meshCollision);
  else
  m_meshDraw.AdjustToFitBoundingBoxMinMax(0, m_meshDraw.GetNrVertices() - 1, m_meshCollision.GetBoundingBoxMin(), m_meshCollision.GetBoundingBoxMax());
...
{
  return m_currState[STATE_Z] <= GetScene()->GetGrid()->GetMax()[2] && m_currState[STATE_Z] >= GetScene()->GetGrid()->GetMin()[2] &&
...
void SimulatorBlimp::AddToMeshCfg(TriMesh &tmesh, const double cfg[]) const
{
    ...
    void SimulatorBlimp::AddToMeshPathCfgs(TriMesh &tmesh, const double cfg1[], const double cfg2[], const double tstart, const double dt, const double tend)
   {
