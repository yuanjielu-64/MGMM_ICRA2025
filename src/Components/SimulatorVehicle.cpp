 ...
 * copyright holder.
 */
#include "Components/SimulatorVehicle.hpp"
...
void SimulatorVehicle::SetupFromParams(Params &params)
{
  ...
  SetMinSteerAngle(Constants::DEG2RAD * params.GetValueAsDouble(Constants::KW_CarMinSteerAngleInDegrees, Constants::RAD2DEG * GetMinSteerAngle()));
  SetMaxSteerAngle(Constants::DEG2RAD * params.GetValueAsDouble(Constants::KW_CarMaxSteerAngleInDegrees, Constants::RAD2DEG * GetMaxSteerAngle()));
...
void SimulatorVehicle::SteerToPosition(const double target[], const bool stop)
{
  ...
  const double v[2] = {cos(m_currState[STATE_THETA] + m_currState[STATE_STEER_ANGLE]), sin(m_currState[STATE_THETA] + m_currState[STATE_STEER_ANGLE])};
